...
from filters import update_filter_options_standardized
...
# Fetch standardized inventory data
@st.cache_data
def fetch_standardized_inventory_data():
    query = """
        SELECT *
...
def process_standardized_inventory_data():
    ...
    if 'df_inventory' not in st.session_state:
        st.session_state['df_inventory'] = fetch_standardized_inventory_data()
    ...
    filter_options = update_filter_options_standardized(df_inventory)

    # Use unique keys for widgets
    selected_brand = st.selectbox(
        "Filter by Brand (Standardized)",
        filter_options["brands"],
        index=filter_options["brands"].index(st.session_state.get("selected_brand_standardized", "ALL")),
        key="selected_brand_standardized_inventory"
    )
    selected_dispensary = st.selectbox(
        "Filter by Dispensary (Standardized)",
        filter_options["dispensaries"],
        index=filter_options["dispensaries"].index(st.session_state.get("selected_dispensary_standardized", "ALL")),
        key="selected_dispensary_standardized_inventory"
    )
    selected_category = st.selectbox(
        "Filter by Category (Standardized)",
        filter_options["categories"],
        index=filter_options["categories"].index(st.session_state.get("selected_category_standardized", "ALL")),
        key="selected_category_standardized_inventory"
    )
    search_term = st.text_input(
        "Search by Product Name (Standardized)",
        value=st.session_state.get("search_standardized_inventory", ""),
        key="search_standardized_inventory_text"
    )
