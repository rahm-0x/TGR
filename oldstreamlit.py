...
# Ensure df_typesense is retrieved correctly
if 'df_typesense' in st.session_state:
    ...
    # Pivot the table so that each snapshot_time becomes a column with inventory levels (Available_Quantity)
    df_pivoted = df_typesense.pivot_table(
        index=['Location', 'Product_Name', 'Price', 'Brand', 'Category'],  # Set your index columns
        columns='snapshot_time',  # Each snapshot time will become a column
        values='Available_Quantity',  # Values will be the available quantities
        aggfunc='first'  # Use 'first' in case there are duplicates
    ).reset_index()  # Reset index to flatten the DataFrame

    # Rename columns to something more readable (optional)
    df_pivoted.columns = [f"{col}" if isinstance(col, tuple) else col for col in df_pivoted.columns]

    # Display the pivoted data
    st.title("Typesense Inventory with Snapshots and Filters")
    st.dataframe(df_pivoted)

    # Optional: You can further manipulate the pivoted data, for example calculating Sales_Since_Yesterday or other columns.
