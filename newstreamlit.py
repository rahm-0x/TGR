...
with main_container:
    ...
    if st.button("Fetch ALL"):
        ...
        st.session_state['typesense_df'] = df_typesense
        ...
        st.session_state['typesense_df'] = df_typesense_tgc
    ...
    if 'df' in st.session_state:
        ...
        if 'Sales_Since_Yesterday' in df.columns:
            price_col_index = df.columns.get_loc("price") + 1
        ...
        if latest_date is not None:
            df_in_stock = df.loc[df[latest_date] > 0]  # Use .loc to avoid the warning
            df_out_of_stock = df.loc[df[latest_date] <= 0]  # Use .loc to avoid the warning
        else:
            df_in_stock = pd.DataFrame()
        ...
        if latest_date is not None:
            df_in_stock = df[df[latest_date] > 0]
            df_out_of_stock = df[df[latest_date] <= 0]
        else:
            st.warning("No recent dates available.")
            df_in_stock = pd.DataFrame()
            df_out_of_stock = pd.DataFrame()
    ...
    if 'curaleaf_df' in st.session_state:
        curaleaf_df = st.session_state['curaleaf_df']
        st.header("Curaleaf Data")
        st.dataframe(curaleaf_df)

    if 'typesense_df' in st.session_state:
        typesense_df = st.session_state['typesense_df']
        st.header("Typesense Data")
        st.dataframe(typesense_df)
